...
static ssize_t _encode_link(const coap_resource_t *resource, char *buf,
                            size_t maxlen, coap_link_encoder_ctx_t *context) {
    (void)_stats_handler; // TODO: remove when i can use more than 2 resources
    (void)_ready_to_duel; // TODO: remove when i can use more than 2 resources
    (void)_start_duel; // TODO: remove when i can use more than 2 resources
    (void)_end_duel;
...
static ssize_t _stats_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len, coap_request_ctx_t *ctx)
{
    ...
    switch (method_flag) {
        ...
        default:
            printf("method flag: %d", method_flag);
    }
...
void notify_observers(void)
{
    ...
    case GCOAP_OBS_INIT_OK:
        ...
        gcoap_obs_send(&buf[0], len, &gs.my_resources[0]);
        break;
    case GCOAP_OBS_INIT_UNUSED:
